...
| File | Role |
|---|---|
| `src/lib/pdf-utils.ts` | **All** PDF logic. 7 exported functions over `pdf-lib`, `mammoth`, `html2pdf.js`. ~240 lines. |
| `src/components/tools/*.tsx` | 6 near-identical form components, one per tool. |
...
### P0-4 · Watermark inputs are unvalidated; raw library assertions reach the user
...
### P0-5 · Non-`Error` throws fail silently
...
### P1-6 · `parsePageRange` silently swallows every invalid segment
...
### P1-7 · Page order and duplicates are discarded
...
### P1-8 · Two distinct filename-mangling bugs
...
### P1-9 · `URL.revokeObjectURL` is called synchronously after `click()`
...
### P1-10 · The same 12-line download block is copy-pasted six times
...
### P1-11 · A cancelled file picker stores `undefined` in `File | null` state
...
### P1-12 · `mergePdf` loses the name of the file that failed
...
### P1-13 · Merge order is the browser's, not the user's
...
### P1-14 · The Split placeholder contradicts the validation
...
### P1-15 · `convertImageToPdf` trusts `file.type` alone
...
### P1-16 · DOCX→PDF silently produces a scanned-looking document
...
### P2-18 · Type safety is switched off
...
### P2-25 · Large unused dependencies ship in the bundle
...
### T-2 · Fixtures — `src/test/fixtures.ts`
...
### T-3 · `parsePageRange` *(export it first — P1-6)*
...
### T-4 · `splitPdf` — *acceptance test for P0-3*
...
### T-8 · `addWatermark` — *acceptance test for P0-4*
...
### T-9 · `downloadBlob` / `stripExtension`
...
### T-10 · Component tests — one file per tool
...
**F-2 · Rotate pages** — `page.setRotation(degrees(n))`. Smallest possible new tool; good first
addition once the test harness exists.

**F-3 · Delete / reorder pages** — falls out of **P1-6**/**P1-7** almost free: reuse the improved
...
**F-8 · Drag-and-drop + a reorderable file list** — properly fixes **P1-13** and applies to every
...
**Phase 1 — Make the app actually work offline** *(smallest diff, largest user impact)*
...
**Phase 2 — Test harness** *(before touching logic)*
...
**Phase 3 — Shared refactor** *(everything below depends on this)*
...
**Phase 5 — Library correctness** *(each with its test)*
...
**Phase 8 — Features**
16. **F-2**, **F-3** (cheap, reuse Phase 5 work) → **F-8** → **F-5**/**F-4** → **F-9** → the rest.
...
- [ ] `npm run lint` — clean with `no-unused-vars` re-enabled.
- [ ] `npm run test` — all specs green; `src/lib/**` coverage ≥ 90%.
- [ ] `npm run build` — succeeds; note the bundle size before/after **P2-25**.
...
- [ ] `grep -rn "as any" src/` — empty.
- [ ] `README.md` claims match reality: Unlock works, offline means offline, DOCX rasterization is
