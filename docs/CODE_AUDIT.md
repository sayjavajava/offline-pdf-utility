...
- `protectPdf` is now **`removePdfPassword`**. `mergePdf` already routes through `loadPdf`, so
  **P1-12** is only about adding the filename to the error, not about restructuring the load.
...
**Phase 3 — Shared refactor** ⬅️ START HERE *(everything below depends on this)*
